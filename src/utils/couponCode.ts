...
export function normalizeCode(input: string): string {
  const cleaned = input.toUpperCase().replace(/[IL]/g, '1').replace(/O/g, '0').replace(/[^0-9A-Z]/g, '');
...
export async function verifyCode(key: CryptoKey, code: string): Promise<{ valid: boolean; issuedAt?: Date }> {
