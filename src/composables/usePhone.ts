...
function normalizePhone(raw: string): string {
  ...
  if (raw.trimStart().startsWith('+7')) digits = digits.slice(1); // '7' из префикса маски — не часть номера
  else if (digits.length >= 11 && (digits[0] === '7' || digits[0] === '8')) digits = digits.slice(1); // вставка с кодом страны
...
export function usePhone() {
  ...
  watch(unmasked, (digits) => { state.phoneDigits = digits; });

  watch(() => state.phoneDigits, (digits) => writeLocal(STORAGE_KEYS.phone, digits));

  const error = computed(() =>
    state.phoneDigits.length > 0 && state.phoneDigits.length < 10
      ? 'Номер неполный'
      : null
  );
